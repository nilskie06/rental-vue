  ...
  <div class="max-w-7xl mx-auto px-4 lg:px-8 py-10">
    ...
    <div class="flex flex-wrap items-center gap-3 mb-8">
      <div class="relative flex-1 min-w-[200px]">
        ...
        <input v-model="search" type="text" placeholder="Search by unit, building, or address..." class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition">
      </div>
      <select v-model="filterType" class="px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-600 focus:ring-2 focus:ring-indigo-500 outline-none">
        ...
        <option>apartment</option><option>studio</option><option>condo</option><option>house</option>
      </select>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="unit in filtered" :key="unit.id" class="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-lg transition group cursor-pointer" @click="$router.push(`/browse/${unit.id}`)">
        <div class="h-48 bg-gradient-to-br from-indigo-100 to-slate-100 flex items-center justify-center text-4xl">🏠</div>
        <div class="p-5">
          ...
          <p class="text-sm text-slate-500 mb-3">{{ unit.bedrooms }} bed · {{ unit.bathrooms }} bath · {{ unit.sqft }} sqft</p>
          <div class="flex items-center justify-between">
            <span class="text-lg font-bold text-indigo-600">₱{{ unit.rent_amount?.toLocaleString() }}<span class="text-sm font-normal text-slate-400">/mo</span></span>
